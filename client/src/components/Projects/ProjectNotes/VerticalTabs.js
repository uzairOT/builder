...
const VerticalTabs = ({ notes, handleSelectedButton , selectedButton}) => {
  return (
    <Stack direction={"column"}  p={1} sx={scrollable} height='calc(92vh - 265px)'>
        ...
        {notes?.map((note, index) => {
          return (
            <Button key={note.id} style={{borderRadius: '7px', height:'100%'}} sx={{ width: "100%", height: "100%", paddingLeft:'32px', '--ButtonGroup-separatorColor': 'white', backgroundColor: selectedButton === index ? '#EBF3F8' : '', }} onClick={()=>{handleSelectedButton(index)}}>
              <Stack maxWidth={"250px"} overflow={"hidden"} >
                <Typography textAlign={"left"} whiteSpace={"nowrap"} textOverflow={"ellipsis"} overflow={"hidden"}  fontSize={'16px'} fontWeight={'700'}  height={'100%'} color={'#202227'}>{note?.subject}</Typography>
                <Typography
                 fontSize={'12px'}
                 fontWeight={'200'}
                 color={'#202227'}
