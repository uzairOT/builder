...
const useCalendarComponents = ({
    ...
    const toolbarComponent = useCallback((props) => (
        isProjectPage ?
            ...
            <CustomToolbar
                ...
                key={toolbarKey}
            />
    ), [isProjectPage, bgColorClient, dailyForecast, setEventView, setMonthEventView, monthEventView, toolbarKey]);

    const dayComponent = useCallback((props) => (
        eventViewRef.current === 'Work Order' ?
            <CustomEventDayTasks {...props} projectId={id} isProjectPage={isProjectPage} /> :
            <CustomEventDayNotes {...props} projectId={id} isProjectPage={isProjectPage} />
    ), [id, isProjectPage, eventViewRef]);

    const weekComponent = useCallback((props) => (
        isDrawerOpen ?
            <CustomEventWeekOnModal projectId={id} isProjectPage={isProjectPage} {...props} /> :
            <CustomEventWeek {...props} projectId={id} isProjectPage={isProjectPage} />
    ), [id, isDrawerOpen, isProjectPage]);

    const MonthComponent = useCallback((props) => { 
    ...
    const eventComponent = useCallback((props) => (
        eventViewRef.current === 'Work Order' ?
            <CustomEventMonthTasks {...props} projectId={id} monthEventView={monthEventView.current} isProjectPage={isProjectPage} /> :
            <CustomEventMonthWeatherNotes {...props} projectId={id} isDrawerOpen={isDrawerOpen} isProjectPage={isProjectPage} />
