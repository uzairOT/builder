import React from "react";
import Button from "@mui/material/Button";

const BuilderProButton = ({ backgroundColor, variant, Icon,iconProps, children, handleOnClick, marginLeft, padding, fontSize, fontFamily,fontWeight, disabled, visibility, alignSelf='left' }) => {
  const themeStyle = {
    button: {
      backgroundColor: variant === "outlined" ? "" : backgroundColor,
      borderRadius: "34px",
      textTransform: "none",
      "&:hover": {
        backgroundColor: variant === "outlined" ? "" : backgroundColor,
      },
      marginLeft: marginLeft ? marginLeft : "15px",
      borderWidth: "1px",
      padding: padding,
      fontSize: fontSize,
      fontFamily: fontFamily,
      fontWeight: fontWeight,
      visibility: visibility ? visibility : 'visible',
    },
  };

  return (
    <Button
      sx={themeStyle.button}
      disableElevation
      variant={variant}
      // startIcon={Icon ? <Icon sx={iconProps} /> : null}
      onClick={handleOnClick}
      disabled={disabled}
      alignSelf={alignSelf}
    >
      {Icon ? <Icon sx={{...iconProps, marginRight:'4px'}} /> : null} {children}
    </Button>
  );
};

export default BuilderProButton;